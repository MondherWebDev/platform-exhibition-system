...
async function populateEventData() {
  try {
    ...
    const globalSettingsDoc = await db.collection('AppSettings').doc('global').get();
    let eventId = 'default';
    ...
    for (const exhibitor of sampleExhibitors) {
      await db.collection('Events').doc(eventId).collection('Exhibitors').doc(exhibitor.id).set({
        ...exhibitor,
        createdAt: new Date(),
        updatedAt: new Date()
      });
    ...
    for (const sponsor of sampleSponsors) {
      await db.collection('Events').doc(eventId).collection('Sponsors').doc(sponsor.id).set({
        ...sponsor,
        createdAt: new Date(),
        updatedAt: new Date()
      });
    ...
    for (const buyer of sampleHostedBuyers) {
      await db.collection('Events').doc(eventId).collection('HostedBuyers').doc(buyer.id).set({
        ...buyer,
        createdAt: new Date(),
        updatedAt: new Date()
      });
    ...
    for (const speaker of sampleSpeakers) {
      await db.collection('Events').doc(eventId).collection('Speakers').doc(speaker.id).set({
        ...speaker,
        createdAt: new Date(),
        updatedAt: new Date()
      });
