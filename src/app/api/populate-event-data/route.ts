...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(`🗑️ Clearing event data for event: ${eventId}`);

    // Clear exhibitors
    console.log('🗑️ Clearing exhibitors...');
    const exhibitorsSnapshot = await getDocs(collection(db, 'Events', eventId, 'Exhibitors'));
    const deleteExhibitorPromises = exhibitorsSnapshot.docs.map(document => deleteDoc(document.ref));
    await Promise.all(deleteExhibitorPromises);
    console.log(`✅ Cleared ${exhibitorsSnapshot.size} exhibitors`);

    // Clear sponsors
    console.log('🗑️ Clearing sponsors...');
    const sponsorsSnapshot = await getDocs(collection(db, 'Events', eventId, 'Sponsors'));
    const deleteSponsorPromises = sponsorsSnapshot.docs.map(document => deleteDoc(document.ref));
    await Promise.all(deleteSponsorPromises);
    console.log(`✅ Cleared ${sponsorsSnapshot.size} sponsors`);

    // Clear hosted buyers
    console.log('🗑️ Clearing hosted buyers...');
    const buyersSnapshot = await getDocs(collection(db, 'Events', eventId, 'HostedBuyers'));
    const deleteBuyerPromises = buyersSnapshot.docs.map(document => deleteDoc(document.ref));
    await Promise.all(deleteBuyerPromises);
    console.log(`✅ Cleared ${buyersSnapshot.size} hosted buyers`);

    // Clear speakers
    console.log('🗑️ Clearing speakers...');
    const speakersSnapshot = await getDocs(collection(db, 'Events', eventId, 'Speakers'));
    const deleteSpeakerPromises = speakersSnapshot.docs.map(document => deleteDoc(document.ref));
    await Promise.all(deleteSpeakerPromises);
    console.log(`✅ Cleared ${speakersSnapshot.size} speakers`);

    return NextResponse.json({
      success: true,
      message: 'Event data cleared successfully!',
      data: {
        exhibitors: exhibitorsSnapshot.size,
        sponsors: sponsorsSnapshot.size,
        hostedBuyers: buyersSnapshot.size,
        speakers: speakersSnapshot.size
      }
    });
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to clear event data' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        exhibitors: sampleExhibitors.length,
        sponsors: sampleSponsors.length,
        hostedBuyers: sampleHostedBuyers.length,
        speakers: sampleSpeakers.length
      }
    });
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to populate event data' },
      { status: 500 }
    );
  }
}
